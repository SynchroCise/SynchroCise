...
const SideBar = ({ currUser, users, isYoutube, drawerWidth }) => {
  const {
    workout,
    openSideBar,
    playWorkoutState,
  ...
  const [workoutTime, setWorkoutTime] = useState(
    workout.exercises[workoutNumber].time
  );
  ...
  useEffect(() => {
    ...
    const timer =
      workoutCounter > 0 &&
      setTimeout(() => setWorkoutCounter((Math.floor((workoutCounter - 0.1)*10)/10).toFixed(1)), 100);
    ...
    >
      <Box mx={2} my={2} height='95%'>
        <Typography variant='body1'>
          {copyRoomCodeButtonMarkup}Room:{" "}
          {roomName.substring(0, 6).toUpperCase()}
        </Typography>
        <Tabs
          ...
          <Grid item>
            {!isYoutube && (
              <ExerciseList
                ...
                nextUpExercise={nextUpExercise}
                data-test='exerciseListComponent'
              />
            )}
          </Grid>
        </TabPanel>
