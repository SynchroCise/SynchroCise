  ...
  const TimerProgressBarMarkup = (
    <React.Fragment>
      ...
      <div>
        <LinearProgress
          variant='determinate'
          value={(workoutCounter / workoutTime) * 100}
        />
      </div>
      <Box display='flex' justifyContent='flex-end'>
        <IconButton onClick={handleStartWorkout} data-test='startWorkoutButton'>
  ...
  const exerciseListMarkup = (
    <React.Fragment>
      ...
      <Typography variant='h5'>
        {workout.exercises[workoutNumber].exercise}
      </Typography>
      <Typography variant='body1'>Next Up</Typography>
      {nextUpExercise &&
      nextUpExercise.length > 1 &&
      typeof nextUpExercise != "string" ? (
        nextUpExercise
          .filter((exercise, index) => {
            return index > workoutNumber;
          })
          .map((exercise, index) => {
            return (
              <Typography
                key={index}
                variant='body2'
                data-test='nextUpExerciseComponent'
              >
                {exercise}
              </Typography>
            );
        ...
        <Typography variant='body2' data-test='nextUpExerciseComponent'>
          {nextUpExercise}
        </Typography>
      )}
    </React.Fragment>
