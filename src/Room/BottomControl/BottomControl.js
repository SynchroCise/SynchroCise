...
const BottomControl = ({
  ...
  const [vid, setVid] = useState(
    room && room.video
      ? room.localParticipant.videoTracks.values().next().value.isTrackEnabled
      : false
  );
  const [mic, setMic] = useState(
    room && room.audio
      ? room.localParticipant.audioTracks.values().next().value.isTrackEnabled
      : false
  );
  ...
  const handleParticipantPage = (pageDelta) => {
    ...
    if (
      all_participants.slice(newPageNum * ppp, newPageNum * ppp + ppp).length >
      0
    ) {
  ...
  return (
    <Grid
      ...
      style={{ width: "100%" }}
      alignItems='center'
      data-test='bottomControlComponent'
    >
      <Grid item xs={4}>
        <Box display='flex' justifyContent='flex-start' alignItems='center'>
          ...
          >
            {vid ? (
              <Videocam data-test='vidOn' />
            ) : (
              <VideocamOff data-test='vidOff' />
            )}
          </IconButton>
          <IconButton
            color='secondary'
      ...
      <Grid item xs={4}>
        ...
        >
          <IconButton
            color='secondary'
            onClick={() => handleParticipantPage(-1)}
            data-test='backPPButton'
          >
            <ArrowBack style={{ fill: "white" }} />
          </IconButton>
          <Typography color='secondary'>
            {" "}
            {participants.length + leaderParticipantIDs.length}/
            {participants.length + leaderParticipantIDs.length} participants{" "}
            {participantPage}{" "}
          </Typography>
          <IconButton
            color='secondary'
            onClick={() => handleParticipantPage(1)}
            data-test='forwardPPButton'
          >
